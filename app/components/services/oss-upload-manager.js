        ...
        function ($q, $state, $timeout, ossSvs2, AuthInfo, Toast, Const, DelayDone, safeApply, settingsSvs) {
            ...
            function addEvents(job) {
                ...
                job.on('complete', function () {
                    concurrency--;
                    checkStart();
                    checkNeedRefreshFileList(job.to.bucket, job.to.key, job.to.oss_url);
            ...
            function checkNeedRefreshFileList(bucket, key, oss_url) {
                ...
                if ($scope.currentInfo.bucket == bucket) {
                    ...
                    if ($scope.currentInfo.key == p) {
                        $scope.$emit('needrefreshfilelists', key, oss_url);
                    }
                }
            }
            ...
            function createUploadJobs(filePaths, bucketInfo, jobsAddingFn) {
                ...
                function dig(absPath, dirPath, callFn) {
                    ...
                    if (fs.statSync(absPath).isDirectory()) {
                        ...
                        //文件
                        var v = 3600;
                        var oss_url = ossSvs2.signatureUrl(bucketInfo.region, bucketInfo.bucket, filePath, v);
