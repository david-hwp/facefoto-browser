angular.module('web')
    .controller('filesCtrl', ['$scope', '$rootScope', '$uibModal', '$timeout', '$translate', 'AuthInfo', 'ossSvs2', 'settingsSvs', 'fileSvs', 'safeApply', 'Toast', 'Dialog', 'Facefoto', 'FacefotoPy',
        function ($scope, $rootScope, $modal, $timeout, $translate, AuthInfo, ossSvs2, settingsSvs, fileSvs, safeApply, Toast, Dialog, Facefoto, FacefotoPy) {
            ...
            var ttid;
            $scope.$on('needrefreshfilelists', function (e, filename, pic_url) {
                ...
                var data = {
                    ...
                    'group_id': $scope.currentInfo.data.groupId
                };
                FacefotoPy.imageFormOssUrl(data).then(function () {
                    Toast.info(T('face.feature.record'));
                }, function (err) {
                    if (err.data.code == 10001) {
                        console.log('no.face.detected: ' + filename)
                        // Toast.warn(T('no.face.detected') + ' [' + filename + ']', 10000)
                    } else {
                        console.log(err);
                    }
                });
            ...
            function refreshCurrent(info) {
                if ($scope.rank == 1 && $scope.allCommunity != null) {
                    ...
                    for (var i = 0; i < allCommunity.length; i++) {
                        if (allCommunity[i].path == info.key) {
                            info.data = allCommunity[i];
                            return;
                        }
                    }
                } else if ($scope.rank == 2 && $scope.allActivity != null) {
                    var allActivity = angular.copy($scope.allActivity);
                    for (var i = 0; i < allActivity.length; i++) {
                        if (allActivity[i].path == info.key) {
                            info.data = allActivity[i];
                            return;
                        }
                    }
                }
            ...
            function addEvents() {
                $scope.$on('ossAddressChange', function (e, addr, forceRefresh) {
                    ...
                    $scope.rank = addr.replace(userAddr, '').split('/').length - 1;
            ...
            function searchSimilar(items) {
                if (items.length > 1) {
                    Toast.warn(T("one.select.item.only"));
                    return
                }
            ...
            function showDeleteFiles(items) {
                $modal.open({
                    ...
                    resolve: {
                        ...
                        rank: function () {
                            return angular.copy($scope.rank);
                        },
