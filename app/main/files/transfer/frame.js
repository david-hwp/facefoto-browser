    ...
    .controller('transferFrameCtrl', ['$scope', '$translate', 'ossUploadManager', 'ossDownloadManager', 'Toast', 'safeApply',
        function ($scope, $translate, ossUploadManager, ossDownloadManager, Toast, safeApply) {
            ...
            angular.extend($scope, {
                ...
                totalNum: {
                    running: 0, total: 0, upDone: 0, downDone: 0,
            ...
            function uploadFilesHandler(filePaths, bucketInfo) {
                ...
                if (filePaths) {
                    angular.forEach(filePaths, function (n) {
                        var suffixIndex = n.lastIndexOf(".");
                        var suffix = n.substring(suffixIndex + 1).toUpperCase();
                        if (suffix != "BMP" && suffix != "JPG" && suffix != "JPEG" && suffix != "PNG") {
                            Toast.error(T("notimage") + ': ' + n);
                        } else {
                            left[i] = n;
                            i++;
                        }
                ...
                if (left.length > 0) {
                    ...
                    ossUploadManager.createUploadJobs(left, bucketInfo, function (isCancelled) {
